...
class VoteReq(Message):
    ...
    def exec(self, node):
        ...
        node.vote_log(time.time(), self.vote_id, 'requested')

    def __str__(self):
        return f'Request Vote for transaction {self.vote_id}'

# Using the vote id, sender id, vote for vote cast
class Vote(Message):
...
# Checking the vote status and abort if participant vote is NO
class Abort(Message):
    def __init__(self, vote_id):
        self.vote_id = vote_id
        ...
        with open(read_path, 'r') as file:
            ...
            for line in lines:
                ...
                if int(vote_id) == self.vote_id:
                    if msg == 'abort':
                        # reply ABORT
                        node.failures_enqueue(self.sender_id, Abort(self.vote_id))
                        return
                    elif msg == 'commit':
                        # reply COMMIT
                        node.failures_enqueue(self.sender_id, Commit(self.vote_id))
                        return
                    else:
                        # append all other possible message to the statuses list
                        statuses.append(msg)
            if 'yes' not in statuses:
                # aborting locally and notify master if the node has not voted yet
